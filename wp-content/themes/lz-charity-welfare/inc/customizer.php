...
function lz_charity_welfare_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting('lz_charity_welfare_show_site_title',array(
       'default' => true,
       'sanitize_callback'	=> 'lz_charity_welfare_sanitize_checkbox'
    ));
	...
	$wp_customize->add_setting('lz_charity_welfare_call',array(
		'default'=> '',
		'sanitize_callback'	=> 'lz_charity_welfare_sanitize_phone_number'
	));	
...
final class LZ_Charity_Welfare_Customize {
	...
	public function sections( $manager ) {
		...
		$manager->add_section(
			new LZ_Charity_Welfare_Customize_Section_Pro(
				$manager,
				'lz_charity_welfare_example_1',
				array(
					'priority' => 9,
					'title'    => esc_html__( 'LZ Charity Pro', 'lz-charity-welfare' ),
					'pro_text' => esc_html__( 'Go Pro','lz-charity-welfare' ),
					'pro_url'  => esc_url( 'https://www.luzuk.com/product/charity-wordpress-theme/' ),
				)
			)
		);
	}
	...
	public function enqueue_control_scripts() {
		...
		wp_enqueue_script( 'lz-charity-welfare-customize-controls', trailingslashit( esc_url(get_template_directory_uri()) ) . '/assets/js/customize-controls.js', array( 'customize-controls' ) );

		wp_enqueue_style( 'lz-charity-welfare-customize-controls', trailingslashit( esc_url(get_template_directory_uri()) ) . '/assets/css/customize-controls.css' );
	}
}
